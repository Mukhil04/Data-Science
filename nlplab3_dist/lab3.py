...
def split_corpus(corpus):
    ...
    a = len(corpus)
    b = math.floor(0.8*a)
    for sentence in range(0, b):
        train.append(corpus[sentence])
    for sentence in range(b, a):
        test.append(corpus[sentence])
    return (train, test)
    raise NotImplementedError
...
def count_ngrams(corpus, n=3):
    ...
    for i in range(len(corpus)):
        for j in range(len(corpus[i])):
            if (corpus[i][j]) in n_grams:
                n_grams[(corpus[i][j])] += 1
            else:
                n_grams[(corpus[i][j])] = 1
            if j < len(corpus[i]) - 1:
                if (corpus[i][j], corpus[i][j+1]) in n_grams:
                    n_grams[(corpus[i][j], corpus[i][j+1])] += 1
                else:
                    n_grams[(corpus[i][j], corpus[i][j+1])] = 1
            if j < len(corpus[i]) - 2:
                if (corpus[i][j], corpus[i][j+1], corpus[i][j+2]) in n_grams:
                    n_grams[(corpus[i][j], corpus[i][j+1], corpus[i][j+2])] += 1
                else:
                    n_grams[(corpus[i][j], corpus[i][j+1], corpus[i][j+2])] = 1
            vocab.add(corpus[i][j])
    return (n_grams, list(vocab))
    raise NotImplementedError
...
def estimate(counts, word, context):
    ...
    words.append(word[0])
    answer = float(counts[tuple(words)]/ counts[tuple(context)])
    return answer
    raise NotImplementedError
...
def vary_ngram(train_corpus, test_corpus, n_gram_orders):
    ...
    '''
    perplexities = {}
    for size in n_gram_orders:
        natr_train, natr_vocab = padded_everygram_pipeline(size, train_corpus)
        natr_test = sum([['<s>'] + x + ['</s>'] for x in test_corpus],[])
        natr_lm = Laplace(size)
        natr_lm.fit(natr_train, natr_vocab)
        perplexities[size] = natr_lm.perplexity(natr_test)
    return perplexities
    raise NotImplementedError
